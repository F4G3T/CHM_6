...
int main() {
    ...
    for (int m = 0; m < N; ++m) {
        ...
        if (amp > 10.0) {  // Порог для значимых частот
    ...
    // Обнуляем высокочастотный шум (omega2) и его эрмитово-сопряженную частоту
    Z_filtered[omega2] = 0;
    ...
    // Проверка, что сигнал остался вещественным (убираем мнимую часть)
    for (int i = 0; i < N; ++i) {
    ...
    // Фильтрация кусочно-постоянного сигнала
    vector<Complex> Z_pw_filtered = Z_pw;
    // Оставляем только низкие частоты (низкочастотная фильтрация)
    int cutoff = 20;
    for (int m = 0; m < N; ++m) {
    ...
    // Сохранение данных для кусочно-постоянного сигнала
